...
public abstract class MicroService implements Runnable { 
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        callbackConcurrentHashMap.putIfAbsent(type, callback);
    }
    ...
    /**
     * this method is used by C3PO, Han-solo and R2D2 to update their unique Diary fields.
     */
    public abstract void UpdateDiary(long value);

    /**
     * The entry point of the micro-service.
     * the microservice accepts messages, and acts upon the callback method.
     * until it receives the TerminationBroadCast, then it terminates.
     */
    @Override
    public final void run() {
        ...
        while(!terminate) {
            ...
            ;
            if(m instanceof TerminationBroadcast){
                terminate();
                break;
            }
            if(m != null) {
                Callback c = callbackConcurrentHashMap.get(m.getClass());
                c.call(m);
                UpdateDiary(System.currentTimeMillis());
            }
        }
        messageBus.unregister(this);
        holder.getInstance().EndCountDown();
        try {
            holder.getInstance().getEndLatch().await();
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        diary.Setter(this);
    ...
    private final ConcurrentHashMap<Class, Callback> callbackConcurrentHashMap;
